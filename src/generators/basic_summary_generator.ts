...
} from '../structures/ips_mandatory_sections';

export class BasicIPSCompositionBuilder {
  ...
  setPatient(patients: TPatient | TPatient[]): this {
    ...
    if (
      patients.length === 0 ||
      !patients.every(patient => patient.resourceType === 'Patient')
    ) {
  ...
  async makeSectionFromSummaryAsync(
    ...
    for (const summaryComposition of summaryCompositions) {
      const resourceEntries =
        summaryComposition?.section?.flatMap(sec => sec.entry || []) ?? [];

      resources.forEach(resource => {
        if (
          resourceEntries?.some(
            entry =>
              entry.reference === `${resource.resourceType}/${resource.id}`
          )
        ) {
          this.resources.add(resource);
    ...
    this.addSectionAsync(
      narrative as TNarrative,
      sectionType,
      sectionResources
    );
    return this;
  ...
  async readBundleAsync(
    ...
    for (const sectionType of Object.values(IPSSections)) {
      const summaryIPSCompositionFilter = useSummaryCompositions
        ? IPSSectionResourceHelper.getSummaryIPSCompositionFilterForSection(
            sectionType
          )
        : undefined;
      const sectionIPSSummary = summaryIPSCompositionFilter
        ? resources.filter(resource => summaryIPSCompositionFilter(resource))
        : [];
      if (sectionIPSSummary.length > 0) {
        consoleLogger.info(
          `Using IPS summary composition for section: ${sectionType}`
        );
        await this.makeSectionFromSummaryAsync(
          sectionType,
          sectionIPSSummary as TComposition[],
          resources as TDomainResource[],
          timezone
        );
        continue;
      }
      const summaryCompositionFilter = useSummaryCompositions
        ? IPSSectionResourceHelper.getSummaryCompositionFilterForSection(
            sectionType
          )
        : undefined;
      const sectionSummary = summaryCompositionFilter
        ? resources.filter(resource => summaryCompositionFilter(resource))
        : [];
      if (sectionSummary.length > 0) {
        consoleLogger.info(
          `Using summary composition for section: ${sectionType}`
        );
        await this.makeSectionFromSummaryAsync(
          sectionType,
          sectionSummary as TComposition[],
          resources as TDomainResource[],
          timezone
        );
      } else {
        consoleLogger.info(
          `Using individual resources for section: ${sectionType}`
        );
        const sectionFilter =
          IPSSectionResourceHelper.getResourceFilterForSection(sectionType);
        const sectionResources = resources.filter(resource =>
          sectionFilter(resource)
        );
        await this.makeSectionAsync(
          sectionType,
          sectionResources as TDomainResource[],
          timezone
        );
      }
    }
  ...
  async buildBundleAsync(
    ...
    if (!this.patients) {
      throw new Error(
        'Patient resource must be set before building the bundle'
      );
    }
    if (!this.patientSummary) {
      throw new Error(
        'Patient summary narrative must be set before building the bundle'
      );
    }
