...
export class DiagnosticResultsTemplate implements ISummaryTemplate {
    ...
    generateNarrative(resources: TDomainResource[], timezone: string | undefined,         now?: Date): string {
        return DiagnosticResultsTemplate.generateStaticNarrative(resources, timezone, now);
    }
                ...
                if (
                  ...
                  <tr>
                    ...
                    <td>${data['source'] ?? '-'}</td>
                        ...
                        // Render top-level observation data only if date is within two years
                  ...
                  <tr>
                    ...
                    // Only render DiagnosticReport if issued is within two years
    ...
    private static generateStaticNarrative(resources: TDomainResource[], timezone: string | undefined, now?: Date): string {
        ...
        const diagnosticReports = this.getDiagnosticReports(resources, twoYearsAgo)
     ...
     */
    private static getObservations(resources: TDomainResource[], twoYearsAgo: Date): Array<TObservation> {
        return resources
            .filter(resourceItem => {
     ...
     */
    private static getDiagnosticReports(resources: TDomainResource[], twoYearsAgo: Date): Array<TDiagnosticReport> {
        return resources
            .filter(resourceItem => {
